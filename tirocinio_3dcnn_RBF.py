...
with tf.device('/cpu:0'): #forza l'uso della cpu in keras
    ...
    def load_video(path, numero, video, task, resize=(IMG_SIZE, IMG_SIZE)) : 
        print(path, "  T",task)
    ...
    def build_feature_extractor():
      feature_extractor_model = "https://tfhub.dev/deepmind/i3d-kinetics-600/1"
      ...
      inputs = keras.Input((None, IMG_SIZE, IMG_SIZE, 3)) #instantiate a Keras tensor
    ...
    def prepare_video(path, numero, video, task):
        ...
        frame_features = np.zeros(shape=(len(frames), NUM_FEATURES), dtype="float32")
        
        # Initialize placeholders to store the features of the current video.
        temp_frame_features = np.zeros(shape=(len(frames), NUM_FEATURES), dtype="float32")
        ...
        if (len(frame_features)>0) :
          for i, batch in enumerate(frames):
              print(frames.shape)
              temp_frame_features[:] = feature_extractor.predict(batch[None, :])
    ...
    print("Standard deviation accuracy for frames: ", statistics.stdev(accuracy))
    print("-------------------------[RBF]-------------------------------")
    #Predict the response for test dataset, for each video
    print("Mean accuracy for video: ", statistics.mean(accuracy_video))
    print("Standard deviation accuracy for video: ", statistics.stdev(accuracy_video))